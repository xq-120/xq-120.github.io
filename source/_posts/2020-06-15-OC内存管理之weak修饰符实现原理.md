...
注意这里的参数是new_referrer，即我们将要把对象指针变量的地址插入到entry哈希表里。
...
再根据旧对象的地址从SideTable的weak 表中获取到weak entry，然后从weak entry的referrers哈希表中把指针变量移除，

再根据新对象的地址从SideTable的weak 表中获取到weak entry，然后将弱引用指针变量登记到weak entry的referrers哈希表中。
