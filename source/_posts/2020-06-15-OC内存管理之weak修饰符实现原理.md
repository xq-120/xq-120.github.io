...
当弱指针变量超出作用域被废弃时，编译器会插入 `objc_destroyWeak` 将该指针变量从对象的weak表里剔除。
...
一个 `__weak` 指针变量如果之前指向了A，后面改为指向B，你是不是得把之前登记的给删除（不删除的话会有什么问题），然后登记到新的对象下，这里就涉及到查找，源码为了提高查找效率又是如何优化的。
...
注意这里的参数是 `new_referrer` ，即我们将要把对象指针变量的地址插入到entry哈希表里。
...
![](https://raw.githubusercontent.com/xq-120/cloudImage/master/pictures/20210209215956.png)
...
[Weak properties are not thread safe when reading](https://bugs.swift.org/browse/SR-192)  不过系统已经解决了。

[[runtime] Thread safety for weak references #1454](https://github.com/apple/swift/pull/1454)
