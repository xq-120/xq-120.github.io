...
push操作就是调用autoreleaseFast添加一个POOL_BOUNDARY哨兵对象并返回哨兵对象所在的内存地址。

第一次执行push的时候返回的其实是一个占位地址EMPTY_POOL_PLACEHOLDER，其他时候就是真正的page上的存储地址了。这里的EMPTY_POOL_PLACEHOLDER是一个性能优化，因为有可能我们虽然写了 `@autoreleasepool{}` 但里面不一定会使用到自动释放的对象，这个时候就没必要创建一个page。
...
5. 添加obj到page上
6. 接第一步如果不等于EMPTY_POOL_PLACEHOLDER，说明当前双链表为空没有page，此时如果obj不等于POOL_BOUNDARY则说明出错了将导致内存泄漏，一般不会走到这里因为autoreleaseFast会过滤判断一层。如果等于POOL_BOUNDARY，则调用setEmptyPoolPlaceholder往TLS上写入占位符EMPTY_POOL_PLACEHOLDER并返回EMPTY_POOL_PLACEHOLDER，第一次push的时候走的就是这里。
...
autoreleaseFast上面已经分析过了，如果是第一次
...
void releaseUntil(id *stop) 
{
    ...
    while (this->next != stop) { //一直释放到stop的位置。
        ...
        if (obj != POOL_BOUNDARY) { //这里很妙，如果obj不是nil也会释放掉。因为有可能page里就没有POOL_BOUNDARY对象，比如在子线程中我就没写@{},然后又产生了自动释放对象，这时page就不会存在POOL_BOUNDARY对象
            objc_release(obj);
        }
    }
