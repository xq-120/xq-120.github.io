...
自动释放池底层是由一个双链表数据结构实现的，双链表的每一个元素被称为AutoreleasePoolPage，每个page可以注册一定数量的对象。在MRC时代我们可以给对象发送一条autorelease消息将其注册到自动释放池里，而在ARC时代我们可以将一个对象赋值给一个__autoreleasing 修饰符的指针变量： `id __autoreleasing obj1 = obj; ` 将其注册到自动释放池里。
...
在子线程中，系统会在创建线程时调用push，在线程销毁时调用pop，因此如果你没有显示创建自动释放池，那么在这期间产生的自动释放对象需要等到线程销毁时才会被释放。
