...
fatal: index-pack failed
```
最后解决办法:从同事那里拷贝源码库(路径为`~/Library/Caches/CocoaPods/Pods/Release`)和源码库的说明书(路径为`~/Library/Caches/CocoaPods/Pods/Specs/Release`),也分别放在自己的上述两个目录下.再pod install解决.  
参考:[无法Pod install第三方库的解决办法]([https://www.jianshu.com/p/e2947189c004](https://www.jianshu.com/p/e2947189c004)
)

ps:
CocoaPods repo仓库的本地存储路径为`~/.cocoapods`,里面包含了所有的第三方库的spec说明书.
...
在清理Mac磁盘存储空间时最好不要删除`~/Library/Caches/CocoaPods`里面的缓存,否则下次pod install如果是新工程的话会从github上重新下载源码库,你懂得.
