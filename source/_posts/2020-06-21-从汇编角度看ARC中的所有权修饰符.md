...
以前这里是将对象注册到自动释放池的，可以参考：[Why __weak object will be added to autorelease pool?](https://stackoverflow.com/questions/40993809/why-weak-object-will-be-added-to-autorelease-pool) 。经过测试发现在MRC中还是将对象注册到自动释放池的。
...
- (void)hello {
    {
        ...
        NSLog(@"%@ 1", self.cat);
        [cat release];
    } //使用weak，cat居然还没有被销毁。被注册到自动释放池里去了
    NSLog(@"%@ 2", self.cat);
    [self.cat hello];
    NSLog(@"%@ hello", self);
}
...
你会发现cat对象被注册了两次。因此使用附有`__weak`修饰符的变量，等价于使用注册到autoreleasepool中的对象。如果你某段代码需要多次使用该weak变量，最好先retain后赋值给一个变量然后使用该变量。要不然每次都会retain+注册到自动释放池。所幸的是现在几乎接触不到MRC的代码，并且在ARC中使用附有`__weak`修饰符的变量，系统也不会retain后再将其注册到autoreleasepool中了，而是在使用后调用release马上释放。
...
继续，当作用域结束时，obj1 变量被废弃，因此编译器插入了一条objc_destroyWeak函数的调用。该方法只是将 obj1的地址从对象的 weak 表中移除，但不会将其置为 nil。
...
dealloc方法就是释放对象自身的内存，以及析构它拥有的所有资源，比如实例变量对象。在MRC下，[super dealloc]必须放在最后。
