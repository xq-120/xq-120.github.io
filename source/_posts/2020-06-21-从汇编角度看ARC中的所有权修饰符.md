...
objc_autoreleaseReturnValue会做一个优化，如果后续紧接着调用objc_retainAutoreleasedReturnValue，则objc_autoreleaseReturnValue不会将生成的对象真的注册到自动释放池中而是直接返回，并往 TLS 写ReturnAtPlus1，当执行objc_retainAutoreleasedReturnValue时，会先从TLS中取值检测到对象是ReturnAtPlus1则不 retain 这样就避免了重复 retain 。

因此将一个以其他单词命名开头的方法创建并返回的对象赋值给一个__strong 修饰符的指针变量时，该对象本应该注册到自动释放池，但在这种情况下系统会做一个优化，该对象将不再注册到自动释放池中，因此能够及时释放对象，起到优化作用。
