...
typedef uint32_t dispatch_lock; //其实就是一个无符号32 位整型。

#define DLOCK_OWNER_MASK			((dispatch_lock)0xfffffffc) //...1100,拥有锁
#define DLOCK_WAITERS_BIT			((dispatch_lock)0x00000001)  //标记是否有等待的线程（置位了就代表有1~多个等待的线程）。
#define DLOCK_FAILED_TRYLOCK_BIT	((dispatch_lock)0x00000002) //trylock失败标志位

#define DLOCK_OWNER_NULL			((dispatch_tid)MACH_PORT_NULL) //当前锁无拥有者
#define _dispatch_tid_self()		((dispatch_tid)_dispatch_thread_port()) //线程id
```
...
将线程id与DLOCK_OWNER_MASK按位与后，返回。

因此 `_dispatch_once_gate_tryenter` 函数的作用如下：

原子比较替换操作保证多线程安全，第一次执行时l->dgo_once也就是onceToken的值默认是 0，必然等于DLOCK_ONCE_UNLOCKED=0，因此将onceToken的值更新为`_dispatch_lock_value_for_self()` （该值里面包含tryenter成功的线程的线程id）。这样当其他线程执行该函数时必然返回 false，tryenter 失败。其他线程将执行_dispatch_once_wait函数，等待 block 执行完成。
...
#endif
	if (likely((dispatch_lock)v == value_self)) return; //值没变，说明没有其他线程尝试执行block，即没有进入等待的线程
...
{
	if (unlikely(!_dispatch_lock_is_locked_by_self(cur))) { //没有拥有锁的线程不能去执行唤醒操作，否则崩溃。不过这里不太可能。
...
({
        ...
        old_v = atomic_load_explicit(((__typeof__(*(_p)) _Atomic *)(_p)), memory_order_relaxed); //获取dgo_once的值
        do {
            {
                if (likely(old_v == DLOCK_ONCE_DONE)) //block已经执行，则直接返回
                {
                    ({
                ...
                new_v = old_v | (uintptr_t)DLOCK_WAITERS_BIT; //按位或DLOCK_WAITERS_BIT，表明当前有等待的线程。
                if (new_v == old_v) //说明之前已经有线程将dgo_once改为new_v了
                ({
                    os_atomic_thread_fence(relaxed);
                    break;
                    __builtin_unreachable();
            ...
            _result = ({
                ...
                _bool _b = atomic_compare_exchange_weak_explicit(((__typeof__(*(_p)) _Atomic *)(_p)), &_r, new_v, memory_order_relaxed, memory_order_relaxed); //dgo_once替换为新值new_v。
                *(&old_v) = _r;
                _b;
            });
...
```c
if (unlikely(_dispatch_lock_is_locked_by((dispatch_lock)old_v, self))) { //可以检测到同一线程嵌套调用。
  DISPATCH_CLIENT_CRASH(0, "trying to lock recursively");
}
...
_dispatch_lock_owner(dispatch_lock lock_value) //根据lock_value，计算出当前拥有锁的线程的线程id,
{
...
通过原子操作读写onceToken的值。当多线程调用dispatch_once时，线程会走tryenter逻辑，由于是原子操作，所以只会有一个线程tryenter成功，tryenter失败的线程进入等待状态。程tryenter成功成功的线程会执行block，执行完后会更新onceToken的值为DLOCK_ONCE_DONE（~0），并且该线程会唤醒等待的线程。当后面又有线程调用dispatch_once时，由于此时onceToken的值为DLOCK_ONCE_DONE，因此直接返回即可。

Q2：static整型变量onceToken的作用是什么？能不能使用一个auto整型变量代替？

onceToken静态整型变量的作用：主要用于标记 block 执行的状态。0 表示 block 未执行，~0表示 block 已执行（block 执行完成后会更新为该值），lock_value（混合了当前加锁的线程ID、是否有等待线程标志等）表示 block 正在执行中。操作onceToken都是使用原子操作函数完成的。通过该静态变量标记 block 的状态再加上原子操作就可以保证即使在多线程环境下 block 也只执行一次。这里并没有使用什么锁这种比较重的同步技术。另外该整型变量必须是 static 的，如果只是一个普通自动变量是起不到作用的，因为自动变量每次调用函数都会初始化。
