...
注意：dispatch_semaphore_signal() 与 dispatch_semaphore_wait() 必须配对使用，否则将导致`EXC_BAD_INSTRUCTION`崩溃。
...
从打印可以看到添加的顺序是无法保证的。但确实保证了同一时刻仅有一个线程可以访问临界区。

疑问：

1. 信号量会减为负数吗？

2. 多线程调用dispatch_semaphore_wait为啥不会导致安全问题？
