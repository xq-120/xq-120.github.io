...
push操作会先添加一个POOL_BOUNDARY哨兵对象，返回的哨兵对象POOL_BOUNDARY所在的存储地址，这个内存地址第一次的时候等于EMPTY_POOL_PLACEHOLDER，其他时候就是真正的page上的存储地址了。这里的EMPTY_POOL_PLACEHOLDER是一个性能优化，因为有可能我们虽然写了 `@autoreleasepool{}` 但里面不一定会使用到自动释放的对象，这个时候就没必要创建一个page。后续pop操作会一直release对象直到POOL_BOUNDARY哨兵对象（该对象就是push操作时返回的）。
