...
3. 其他情况比如自定义的并发队列则走正常的_dispatch_sync_invoke_and_complete逻辑，直接在当前线程执行block就完事了。
		...
		dispatch_queue_class_t dqu, uintptr_t dc_flags)
{
	...
	if (unlikely(!dq->do_targetq)) { //全局队列满足条件走下面的逻辑，不会有死锁问题
