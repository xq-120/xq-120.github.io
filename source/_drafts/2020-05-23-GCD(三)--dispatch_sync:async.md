...
1. dispatch_sync+串行队列工作流程，传入的任务是如何保存的，又是如何被执行的，特别是队列是一个较高层级的queue时任务的执行过程又是怎样的
2. dispatch_sync+并发队列工作流程，传入的任务是如何保存的，又是如何被执行的，特别是队列是一个较高层级的queue时任务的执行过程又是怎样的
3. dispatch_sync死锁检测是如何检测的，是否会有检测不到的情况？
4. dispatch_async+串行队列工作流程，传入的任务是如何保存的，又是如何被执行的，特别是队列是一个较高层级的queue时任务的执行过程又是怎样的
5. dispatch_async+并发队列工作流程，传入的任务是如何保存的，又是如何被执行的，特别是队列是一个较高层级的queue时任务的执行过程又是怎样的
6. 底层的线程池是如何维护和管理的
		...
		dispatch_function_t func, uintptr_t dc_flags)
{
	if (likely(dq->dq_width == 1)) { //串行队列走下面_dispatch_barrier_sync_f逻辑
		return _dispatch_barrier_sync_f(dq, ctxt, func, dc_flags); //里面如果try_acquire_barrier失败会进入_dispatch_sync_f_slow
	}
...
libdispatch中创建并分配线程来执行任务块的过程中，线程/队列对资源的操作满足上述前三个条件，那么如果再满足第四个条件，必然会发生死锁，所以循序等待是我们检查代码的重点。而在GCD中满足下面两个条件即会形成循环等待的情形：
...
但这种检测办法并不能检测出所有死锁的情况，它只能检测出**同一线程--同一队列**的情况，其他情况就检测不到了，比如下面的代码：会形成一个循环等待但并不是同一线程--同一队列的情况，所以检测不出来，也就不会崩溃。
...
- (void)test_dispatch_sync_serial_deadlock_but_not_checked {
    ...
    dispatch_async(sQ1, ^{ //假设在线程A执行
        NSLog(@"Enter");
        dispatch_sync(dispatch_get_main_queue(), ^{ //在主线程执行
            NSLog(@"---");
            dispatch_sync(sQ1, ^{ //非同一线程--同一队列情况，所以这里检测不出来。因为sQ1的dq_state里保存的是线程A的id，但这里的wait线程主线程，二者的id不一致，所以检测不出来。但实际上已经是死锁了。
...
//可以检查出来的情况
- (void)test_dispatch_sync_serial_deadlock0 {
    dispatch_queue_t queue = dispatch_queue_create("com.xq.queue", DISPATCH_QUEUE_SERIAL);
    dispatch_sync(queue, ^{
        NSLog(@"current thread = %@", [NSThread currentThread]);
        dispatch_queue_t mainQ = dispatch_get_main_queue();
        NSLog(@"---");
        dispatch_sync(mainQ, ^{ //main_queue的dq_state里肯定保存了主线程ID，然后这里的wait线程又是主线程，所以检测出来了。于是崩溃。
            NSLog(@"current thread = %@", [NSThread currentThread]);
        });
        NSLog(@"+++");
    });
